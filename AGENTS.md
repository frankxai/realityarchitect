...
1. If they haven't self-diagnosed, point them to `/assess` (or ask the five questions in `app/assess/` `MOVES`).
2. Identify the **first** move scored below "locked in." That is the gap.
3. Open the matching template in `starter/` and adapt it to their context — don't hand them all five.
4. Keep the guardrail: every recommendation must cash out in a buildable artifact (a vault, a spec, an agent, a loop,
